...
(vei_valor * (alug_dt_real_devolucao - alug_dt_locacao))
FROM veiculo
INNER JOIN aluguel on vei_id = fk_alug_veiculo
WHERE (alug_dt_real_devolucao - alug_dt_prev_devolucao) = 0;

-- Selecione o supervisor
SELECT * FROM funcionario WHERE fk_func_supervisor IS NULL;

-- Selecione nome do funcionario , do veiculo  e nome do cliente
-- que alugou um carro gol e entregou em atraso
SELECT vei_modelo, func_nome, cli_nome,  
(vei_valor * (alug_dt_real_devolucao - alug_dt_locacao)) AS 'Valor do aluguel',
(alug_dt_real_devolucao - alug_dt_prev_devolucao) AS 'Dias em atraso',
cli_renda AS 'Renda do cliente',
(cli_renda * 1.5) AS 'Renda do cliente com 50% de aumento'
FROM veiculo
INNER JOIN aluguel on vei_id = fk_alug_veiculo 
INNER JOIN cliente on cli_id = fk_alug_cliente 
INNER JOIN funcionario on func_id = fk_alug_funcionario
INNER JOIN bairro on bairro_id  = fk_func_bairro
WHERE vei_modelo = 'Gol'
and
(alug_dt_real_devolucao - alug_dt_prev_devolucao) > 0;